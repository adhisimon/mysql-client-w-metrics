...
// const metrics = async () => await register.metrics();
...
class MySQLClientWMetrics {
  ...
  constructor (poolOptions, poolId) {
    ...
    connectionLimitGauge.set({ pool_id: this.poolId }, connectionLimit);

    this.highestConnectionQueueLength = 0;

    const updateMetrics = () => {
      activeConnectionGauge.set({ pool_id: poolId }, this.poolCallbackStyle._allConnections?.length || 0);
      freeConnectionGauge.set({ pool_id: poolId }, this.poolCallbackStyle._freeConnections?.length || 0);

      const queueLength = this.poolCallbackStyle._connectionQueue?.length || 0;
      connectionQueueLengthGauge.set({ pool_id: poolId }, queueLength);
      this.highestConnectionQueueLength = Math.max(this.highestConnectionQueueLength, queueLength);
      highestConnectionQueueLengthGauge.set({ pool_id: poolId }, this.highestConnectionQueueLength);
    };
